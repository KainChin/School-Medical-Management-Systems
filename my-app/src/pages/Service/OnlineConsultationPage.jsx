...
//                   <CheckCircle /> Đặt lịch thành công với y tá <strong>{nurses.find(n => n.id === Number(selectedNurseId))?.name}</strong>{" "}
...
const OnlineConsultationPage = () => {
  ...
  const occupiedSlots = useMemo(() => {
    if (!selectedNurseId || !selectedDate) return [];
    return appointments
      .filter((app) => {
        if (!app.appointmentDate || !app.nurseId) return false;
        const [date, time] = app.appointmentDate.split("T");
        return (
          Number(app.nurseId) === Number(selectedNurseId) &&
          date === selectedDate
        );
      })
      .map((app) => app.appointmentDate.split("T")[1].slice(0, 5));
  }, [appointments, selectedNurseId, selectedDate]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      selectedDate < today ||
      (selectedDate === today && selectedSlot <= currentTime)
    ) {
      return setError("Chọn thời điểm hợp lệ.");
    }
  ...
  return (
    <>
      ...
      <div className="oc-banner">
        <div className="oc-banner-card">
          <div className="oc-banner-content">
            <h2 className="oc-banner-title">Gọi video trực tuyến với y tá</h2>
            {[
              "Y tá và trợ lý y tá chuyên trách trong trường, kinh nghiệm cao",
              "Hỗ trợ khám & chăm sóc y tế tại trường học",
              "Tư vấn sức khỏe cá nhân hóa",
            ...
            <div className="oc-contact">
              <span>
                Liên hệ ngay qua <strong>số điện thoại</strong>
              </span>
              <a href="tel:19002115" className="oc-hotline-link">
                <PhoneCall />
                19002115
        ...
        <div className="oc-consultation-wrapper">
          <div className="oc-booking">
            ...
            {submitted ? (
              <div className="oc-success">
                <div className="oc-success-message">
                  <CheckCircle /> Đặt lịch thành công với y tá{" "}
                  <strong>
                    {
                      nurses.find((n) => n.id === Number(selectedNurseId))
                        ?.name
                    }
                  </strong>{" "}
                  vào <strong>{selectedDate} {selectedSlot}</strong>
                </div>
                <button onClick={reset} className="oc-reset-button">
                  Đặt lại
                </button>
                    ...
                    <div className="oc-slots">
                      {slots.map((s) => {
                        const currentTime = new Date()
                          ...
                          .slice(0, 5);
                        const isPast =
                          selectedDate === today && s <= currentTime;
                        const isBooked = occupiedSlots.includes(s);
                        const disabledSlot = isPast || isBooked;
                        const isSelected = selectedSlot === s;
                        ...
                        return (
                          <button
                            ...
                            disabled={disabledSlot}
                            onClick={() =>
                              !disabledSlot && setSelectedSlot(s)
                            }
                            className={btnClass}
                            style={
                              isBooked
                            ...
                            title={
                              isBooked ? "Khung giờ đã được đặt" : ""
                            }
                          >
                            {s}
                            {isBooked && (
